...
def auto_send_view_status(message):
    try:
        ...
        is_auto_ON = (books_library.get_auto_status(userId) == 1)
...
def process_change_book(message):
    try:
        ...
        books_list = books_library.get_user_books(userId)
        if new_book in books_list:
            # fh.db.update_current_book(userId, chatid, new_book)
            books_library.update_current_book(userId, chatid, new_book)
...
def listener(message):
    try:
        ...
        user_id = message.from_user.id
        tb.send_chat_action(chat_id, 'typing')
        next_portion = book_reader.get_next_portion(user_id)
        # tb.send_message(chat_id, fh.get_next_portion(user_id), reply_markup=remove_markup)
...
def now_reading_handler(message):
    try:
        ...
        tb.send_chat_action(chat_id, 'typing')
        # book_name = fh.db.get_current_book(user_id).replace(str(user_id) + '_', '')
        book_name = books_library.get_current_book(user_id)
        book_name = book_name.replace(str(user_id) + '_', '')
        if book_name != -1:
...
def auto_send_portions():
    ...
    for item in send_list:
        try:
            ...
            chat_id = item[1]
            tb.send_chat_action(chat_id, 'typing')
            # tb.send_message(chat_id, fh.get_next_portion(userId))
            tb.send_message(chat_id, book_reader.get_next_portion(user_id))
        except Exception as e:
            logger.error(e)
    return 0
